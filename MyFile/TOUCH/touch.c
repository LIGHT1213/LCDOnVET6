...
void TP_Save_Adjdata(void)
{
    ...
    //AT24CXX_WriteOneByte(SAVE_ADDR_BASE + 12, tp_dev.touchtype);
		Flash_Temp[4*3+1]=tp_dev.touchtype;
		
    //temp = 0X0A; //标记校准过了
		//AT24CXX_WriteOneByte(SAVE_ADDR_BASE + 13, temp);
		Flash_Temp[4*3+2]=0X0A;
		
		W25QXX_Write((u8*)Flash_Temp,0,24);		//从第0个地址处开始,写入SIZE长度的数据
}
...
{					  
    s32 tempfac;
	  unsigned char Flash_Temp[24];
	   ...
	   W25QXX_Read(Flash_Temp,0,24);					//从第0个地址处开始,读出SIZE个字节
	
    tempfac = Flash_Temp[4*3+2]; //读取标记字,看是否校准过！
    ...
    if(tempfac == 0X0A) //触摸屏已经校准过了
    {
        ...
        //tp_dev.touchtype = AT24CXX_ReadOneByte(SAVE_ADDR_BASE + 12); //读取触屏类型标记
			  ...
			  tp_dev.touchtype=Flash_Temp[4*3+1];
			...
			{			   
				...
				case 4:	 //全部四个点已经得到
					...
					if(fac<0.95||fac>1.05||d1==0||d2==0)//不合格
					{
						...
						LCD_ShowString(30,50,200,16,16,"no");
 						continue;
					}
